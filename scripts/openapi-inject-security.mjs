...
const FORBIDDEN_SCHEMA = {
  type: 'object',
  description:
    'Returned when the caller is authenticated but lacks the required PRO entitlement tier.',
  properties: {
...
const FORBIDDEN_RESPONSE = {
  description: 'Authenticated caller lacks the required PRO entitlement.',
  content: {
    'application/json': {
...
const YAML_FORBIDDEN_SCHEMA = [
  ...
  '            required:',
  '                - error',
  '            description: Returned when the caller is authenticated but lacks the required PRO entitlement tier.',
];
...
function ensureYamlForbiddenResponse(lines, path, responseLines = YAML_FORBIDDEN_RESPONSE) {
  const op = findYamlOperationRange(lines, path);
  if (!op) return false;
  const has403 = lines.some((line, index) => (
    index > op.start && index < op.end && line === '                "403":'
  ));
